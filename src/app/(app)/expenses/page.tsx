...
export default function ExpensesPage() {
  ...
  const clearForm = () => {
    setEditing(null);
    setForm({ title: "", amount: "", date: "", vendor: "", category: "Food", paymentMethod: "Credit Card", notes: "", linkedReceiptId: "" });
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" onClick={() => setAmountSort(amountSort === "asc" ? "desc" : "asc")}>
      ...
      <Card className="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-3">
        <Input placeholder="Search vendor, title, notes..." value={expenseQuery} onChange={(e) => setExpenseQuery(e.target.value)} />
        <Input type="date" value={form.date} onChange={(e) => setForm((current) => ({ ...current, date: e.target.value }))} />
        <Select value={form.linkedReceiptId} onChange={(e) => setForm((current) => ({ ...current, linkedReceiptId: e.target.value }))}>
          <option value="">No linked receipt</option>
          {receipts.map((receipt) => <option key={receipt.id} value={receipt.id}>{receipt.merchant}</option>)}
        </Select>
        <Input placeholder="Expense title" value={form.title} onChange={(e) => setForm((current) => ({ ...current, title: e.target.value }))} />
        <Input placeholder="Amount" type="number" value={form.amount} onChange={(e) => setForm((current) => ({ ...current, amount: e.target.value }))} />
        <Input placeholder="Vendor" value={form.vendor} onChange={(e) => setForm((current) => ({ ...current, vendor: e.target.value }))} />
        ...
        <Select value={form.paymentMethod} onChange={(e) => setForm((current) => ({ ...current, paymentMethod: e.target.value as PaymentMethod }))}>
        ...
        <div className="flex gap-2 md:col-span-2 xl:col-span-3">
          <Button onClick={submit}>{editing ? "Save expense" : "Create expense"}</Button>
          ...
          <Card key={expense.id} className="p-5">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <div className="mt-1 text-sm text-muted-foreground">{expense.vendor} · {expense.category} · {expense.paymentMethod}</div>
              ...
              <div className="flex flex-wrap gap-2">
                <Button variant="outline" onClick={() => { setEditing(expense); setForm({ title: expense.title, amount: String(expense.amount), date: expense.date, vendor: expense.vendor, category: expense.category, paymentMethod: expense.paymentMethod, notes: expense.notes, linkedReceiptId: expense.linkedReceiptId ?? "" }); }}>Edit</Button>
